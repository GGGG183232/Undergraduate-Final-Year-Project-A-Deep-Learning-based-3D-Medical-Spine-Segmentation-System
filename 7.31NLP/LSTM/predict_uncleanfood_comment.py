...
path = 'data/preprocess/preprocess_2.csv'
...
# 保存dataframe为csv
train_filter.to_csv('E:\savefromxftp\\7.31MON_NLP\data\preprocess\preprocess_2.csv', index=True, header=False)

# 读入停用词表
stopwords = list()
with open('E:\savefromxftp\\7.31MON_NLP\data\stopwords\哈工大停用词表.txt', encoding='utf-8') as f:  # todo: 之前一直报错就是没用utf-8
...
sub = test_file.copy()
sub['label'] = test_mask
sub[['id', 'comment', 'label']].to_csv('E:\savefromxftp\\7.31MON_NLP\data\preprocess\preprocess_2.csv', index=None)
